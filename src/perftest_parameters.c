...
static void usage(const char *argv0, VerbType verb, TestType tst, int connection_type)
{
	...
	if (tst != FS_RATE) {
		...
		if (rocm_memory_supported()) {
			...
			printf("      --use_rocm_bus_id=<rocm full BUS id>");
			printf(" Use ROCm specific device, based on its full PCIe address, for GPUDirect RDMA testing\n");
...
int parser(struct perftest_parameters *user_param,char *argv[], int argc)
{
	...
	static int use_rocm_bus_id_flag = 0;
	...
	while (1) {
		...
		static const struct option long_options[] = {
			...
			{ .name = "use_rocm_bus_id",	.has_arg = 1, .flag = &use_rocm_bus_id_flag, .val = 1},
		...
		switch (c) {
			...
			case 0: /* required for long options to work. */
				...
				if (((use_cuda_flag || use_cuda_bus_id_flag) && !cuda_memory_supported()) ||
				    ...
				    ((use_rocm_flag || use_rocm_bus_id_flag) && !rocm_memory_supported()) ||
				...
				if (user_param->memory_type != MEMORY_HOST &&
				    (mmap_file_flag || use_neuron_flag || use_hl_flag || 
					 ((use_rocm_flag || use_rocm_bus_id_flag) && user_param->memory_type != MEMORY_ROCM) ||
				...
				if (use_rocm_bus_id_flag) {
					user_param->rocm_device_bus_id = strdup(optarg);
					printf("Got PCIe address of: %s\n", user_param->rocm_device_bus_id);
					user_param->memory_type = MEMORY_ROCM;
					user_param->memory_create = rocm_memory_create;
					use_rocm_bus_id_flag = 0;
				}
